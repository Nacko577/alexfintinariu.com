...
// Returns { entry, groupName } where groupName is typically the division (e.g. "AFC North")
...
export async function GET() {
  try {
    // Standings (division/conference trees)
    const standingsUrl = `${ESPN_WEB}/sports/${SPORT}/${LEAGUE}/standings?level=3`;

    // Scoreboard around today (enough window to get last + next)
    ...
    const groupName = found?.groupName ?? null;

    const wins = entry ? statValue(entry, "wins") : null;
    const losses = entry ? statValue(entry, "losses") : null;
    const ties = entry ? statValue(entry, "ties") : null;

    // ESPN sometimes provides division rank / playoff seed
    ...
    const res = NextResponse.json({
      record,
      divisionRank,
      groupName, // usually "AFC North" or similar
      lastGame,
      nextGame,
    ...
    return NextResponse.json({ error: e?.message ?? "Failed to fetch Ravens data" }, { status: 500 });
  }
}